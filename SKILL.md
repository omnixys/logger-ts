<!-- repository: packages/ts/logger | kind: PACKAGE | stack: ts-package -->

# logger — Skill: Package Development

> Workflow for logger (packages/ts/logger). Execute this workflow before, during, and
> after changes in this repository.

## Repository Facts

- Kind: Shared Package
- Package: `@omnixys/logger-ts` (version: 3.2.2)
- Runtime: Node >=20 (pnpm >=10.33.0)
- Description: Omnixys structured logger for TypeScript.
- Architecture: src/ logger
...
- This package is published (`@omnixys/logger-ts`); consumers pin versions and rely on SemVer.
